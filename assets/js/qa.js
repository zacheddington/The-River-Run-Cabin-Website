...
(function () {
  ...
  /**
   * Wrap table elements in scrollable container for mobile responsiveness
   * @param {string} html - HTML string to process
   * @returns {string} Processed HTML with wrapped tables
   */
  ...
  qaList.forEach((item) => {
    ...
    const answerHtml = wrapTablesForScroll(item.a);

    div.innerHTML = `<h2>${item.q}</h2><div class="qa-answer">${answerHtml}</div>`;
